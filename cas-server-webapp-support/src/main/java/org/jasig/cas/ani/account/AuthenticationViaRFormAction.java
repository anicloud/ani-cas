...
public class AuthenticationViaRFormAction {
     ...
     * @since 4.1.0
     */
    public final Event submit(final RequestContext context, final Credential credential, final MessageContext messageContext) throws Exception {
        ...
        try {
            ...
            WebUtils.putTicketGrantingTicketInScopes(context, TGTId);
            final Service service = WebUtils.getService(context);
            final ServiceTicket serviceTicket = this.centralAuthenticationService.grantServiceTicket(TGTId,service);
            WebUtils.putServiceTicketInRequestScope(context,serviceTicket);
            context.getFlowScope().put("ticket", serviceTicket);
            context.getFlowScope().put("ret", 1);
            context.getFlowScope().put("msg", "登录成功！");
    ...
    public String doBind(RequestContext context , Credential credential){
